...
时钟约定（PREREG #8 锚定，用户 2026-07-18 依 LMGen 源码裁定）：步 s 覆盖
[s·τ, (s+1)·τ)。标签行 step s 对应"在线系统刚接收完对方音频帧 s 的决策状态"，
观测截止 = (s+1)·τ——即行内 t 列（步末时刻）恰为观测截止时刻。各表征应读的行由
mve/alignment.py **唯一**定义（acts 读行 s+1，Mimi/hazard/声学读行 s），本文件
不另行约定表征行映射。
T1 的 δ 前瞻为整步平移：y[s] = "合格 ONSET_agent 落在步 s + δ/τ 内"；δ=240 ms
（τ=80 ms）即步 s+3——标签步索引整 3 步，相对观测截止 (s+1)·τ 的净前瞻为
[160, 240) ms（见 PREREG #8 登记）。
注：T1 条件集（步 s 处于 LISTEN/GAP）与 T4 行定位（对方段末落在步 s）按步 s
自身窗口判定；该窗口在 #8 锚定下**完全位于观测截止之内**，且对四路表征对称。
"""
