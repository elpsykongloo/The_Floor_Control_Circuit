...
时钟约定（PREREG #7 对齐后）：步 s 覆盖 [s·τ, (s+1)·τ)。标签行 step s 的语义为
"决策发生在 s·τ、只允许观测 ≤ s·τ 的音频"；各表征应读的行由 mve/alignment.py
**唯一**定义（acts 读行 s，Mimi/hazard/声学读行 s−1），本文件不再另行约定表征
观测截止。行内 t 列 = 步末时刻 (s+1)·τ，仅作时间戳。
T1 的 δ 前瞻为整步平移：y[s] = "合格 ONSET_agent 落在步 s + δ/τ 内"；δ=240 ms
（τ=80 ms）即步 s+3，相对观测截止 s·τ 的前瞻窗为 [240, 320) ms。
注（文档/02 第六轮登记）：T1 的条件集（步 s 处于 LISTEN/GAP）与 T4 的行定位
（对方段末落在步 s）按步 s 自身窗口判定——这是冻结任务本体的定义性选择，对四路
表征完全对称，不构成表征间比较（G1 优势）的泄漏。
"""
