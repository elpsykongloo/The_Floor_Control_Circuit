...
- **E1 几何解剖支线（PREREG #32/#35，探索性、已实现待本机运行）**：用户 2026-07-23 授权对 G2 有效秩失败做非裁决解剖。核心恒等式：T4 二分类探针的评估 AUC 0.8348 本身就是原始空间单方向 v\*=(w/σ) 的一维投影 AUC（正式 rank-1 PCA 仅 0.575），故有效秩 128 度量的是 v\* 与方差主轴的错向而非信息维数。`wp_e1_geometry_autopsy.py`（协议 v2，绑定源码/行域/fit 摘要）四阶段：directions（零激活方向族）→ spectrum（能量谱/INLP 冗余谱/LEACE 自检/补空间重训/Mimi 主轴鉴定/转向包 `steering_L{29,30,31}.npz`，e1x-directions-v1 兼容、可供 `wp_e2_lite_plan.py --directions-npz` 消费）→ trajectory（事件锁定轨迹，sup-t 同时带 + 连续 ≥3 步）→ finalize（八行判读矩阵）。**#35 核心纪律：擦除训练集 diff-in-means 后一轮归零是数学必然（LEACE），"崩塌轮数"不可作判据；冗余用 INLP r₁ 保留率（α ≤0.3 / β ≥0.7）判读，均值差擦除仅作实现自检**。G2=fail 不变；判读矩阵、E2 方向注入升级与换位点条件化见 文档/04 v0.2。
