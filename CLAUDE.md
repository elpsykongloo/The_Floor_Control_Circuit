...
- **E1 几何解剖支线（PREREG #32，探索性、已实现待本机运行）**：用户 2026-07-23 授权对 G2 有效秩失败做非裁决解剖。核心恒等式：T4 二分类探针的评估 AUC 0.8348 本身就是原始空间单方向 v\*=(w/σ) 的一维投影 AUC（正式 rank-1 PCA 仅 0.575），故有效秩 128 度量的是 v\* 与方差主轴的错向而非信息维数。`wp_e1_geometry_autopsy.py` 四阶段：directions（零激活方向族，用 `work/fits` 断点）→ spectrum（能量谱/迭代均值差擦除/补空间重训/Mimi 主轴鉴定/转向向量包，单卡断点续跑）→ trajectory（评估集事件锁定轨迹）→ finalize（判读矩阵）。实现纪律：迭代擦除方向必须用训练集 diff-in-means，探针方向擦除有固定点病理（合成测试固化）。G2=fail 不变；判读矩阵、E2 方向注入升级与换位点条件化见 文档/04。
