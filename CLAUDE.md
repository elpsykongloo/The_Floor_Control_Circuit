...
- **E1 前置并行（PREREG #22/#24）**：acoustic 只读取冻结前 240 s，支持原子 NPY 与 `--acoustic-workers`；parity 可在训练前 20 会话的标签和 40 路 Zarr 就绪后提前运行。声学 1000/1000 已完成、GPU1 parity 已通过。缺失标签逐通道载入完整波形以收紧峰值内存；单会话压力探针未拖慢摄取，正式调度改为摄取 3 worker 与标签 4 个互斥分片重叠。
