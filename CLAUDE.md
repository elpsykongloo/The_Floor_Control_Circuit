...
- **E1 正式收口与探索转向（PREREG #30/#33/#34/#39）**：Moshi R1 正式 G2 = **fail**（T4@L29 优势 +0.0659 [+0.0581,+0.0743]；top-3 交集与方向余弦通过，唯一失败项 = PCA 有效秩 128 ≫ 16；#30 事后诊断过线位置 57–84、前 16 PC 仅解释 38–45% 方差）。用户裁决开启探索轨道，G2=fail 原样保留、探索/正式分轨登记。**E1-X 已闭合**：七阶段全部完成；T4 探针优势贯穿 0–2000 ms 先知曲线（2000 ms：0.6816 vs Mimi 0.6554，优势 +0.0262 [+0.0152,+0.0369]），L29 相对 Mimi-GRU 优势 +0.0464 [+0.0378,+0.0559]，Mimi 线性重建后的残差 AUC≈0.699；T4 与 Mimi 轨迹都在 −2000 ms 窗口边界已分叉，内部领先量登记为 0 ms（左删失，不能解释为精确起点）；方向跨种子 |cos|≥0.975，前 16 PC 只承载约 21% 方向质量。**E2-lite 已闭合且探索性强阳性**：20 会话×13 条件=260/260、缺失 0；主方向发声占比随 α 单调变化（ρ=+0.871），α=−4 为 −0.1208（20/20 负向）、α=+4 为 +0.3000（20/20 正向），随机方向效应接近零；重叠/抢话随 α 增大，响应延迟无单调变化。结论收窄为“T4@L29 方向具有方向级充分性并主要控制发言倾向/侵入性”，正式 G3 仍未裁决，换位点、消融与组件级定位继续无条件执行。优化执行在完整 13 条件会话上 13/13 逐位一致并加速 3.45 倍。`causal_eval` 25% 侧继续未读取；完整报告见 `reports/e1x_e2_lite_综合结论.{md,html}`。
- **E2 深挖与确认臂（PREREG #40，已实现待本机运行）**：四件套全部探索性、基线复用 E2-lite baseline（同会话同种子）、不触 causal_eval。(a) `wp_e2_lite_deep.py` 零 GPU：语境三分/SDT（d′ 与 criterion c；判读预登记 = c 单调而 d′ 保持 → 判据旋钮）/文本质量（PAD id 假定 3，以 baseline top-token 核验）/与同 20 会话人类双方的 gap-overlap JSD；掩码走 `e1x/mask_cache.py` 内容寻址缓存。(b) E2 确认臂：`wp_e2_confirm_plan.py`（投影 μ/σ 按层重算、事件门由用户通道 VAD 预生成）→ `runners/moshi/run_steer_confirm.py`（仅 eager 参考语义 NO_CUDA_GRAPH=1；clamp 用 float32 钳制数学）→ `wp_e2_confirm_analyze.py`（必要性/时间/轴/层四组 + E2-lite 连续 ±4 参照列）。(c) 先知曲线 shift 网格扩至 4.96 s；断点含 min_anchor_step 守卫，网格变更旧档自动重算。(d) `wp_e2_lite_r2_probe.py`：baseline 缓存激活的 R2 零样本读出（对齐声明：缓存行 i ≙ 决策步 i；agent 掩码平移 first_emitted·τ）。缓存归档议题已取消（用户 2026-07-24：存储充足）。
