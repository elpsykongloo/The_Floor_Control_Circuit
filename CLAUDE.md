...
- **E1 几何解剖支线（PREREG #32/#35/#36，探索性、已实现待本机运行）**：用户 2026-07-23 授权对 G2 有效秩失败做非裁决解剖。核心恒等式：T4 二分类探针的评估 AUC 0.8348 本身就是原始空间单方向 v\*=(w/σ) 的一维投影 AUC（正式 rank-1 PCA 仅 0.575），故有效秩 128 度量的是 v\* 与方差主轴的错向而非信息维数。`wp_e1_geometry_autopsy.py`（协议 v2，绑定 script/probe_gpu/grid/engine/alignment 源码 + 行域 + 逐任务 fit 摘要）四阶段：directions（零激活方向族）→ spectrum（能量谱/**特征折冗余**/**白化秩-1 确认**/补空间重训/Mimi 主轴鉴定/转向包 `steering_L{29,30,31}.npz`，e1x-directions-v1 兼容、随机方向数=3、proj_std=train pooled、可供 `wp_e2_lite_plan.py --directions-npz` 消费）→ trajectory（事件锁定轨迹，sup-t 同时带 + 连续 ≥3 步）→ finalize（八行判读矩阵，feature_split 替代旧 inlp 行）。**#36 核心纪律：均值信号线性擦除秩恒为 1（LEACE）、方向剔除法（INLP r₁）受协方差 Σ⁻¹d 旋转混淆会把单均值信号误判为厚方向束——二者均已撤销为判据；冗余改协方差无关的特征折（α 分布式 ≥0.5 / β 局部化 <0.25），白化剔除唯一判别方向后残差→0.5 确认二分类线性读出秩=1（描述性硬门）；均值投影自检/白化残差偏高/任何未收敛拟合均拒绝出矩阵**。G2=fail 不变；判读矩阵、E2 方向注入升级与换位点条件化见 文档/04 v0.3。
