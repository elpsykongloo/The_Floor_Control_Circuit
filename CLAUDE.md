...
- **E1 前置并行（PREREG #22/#24/#25）**：acoustic 只读取冻结前 240 s，支持原子 NPY 与 `--acoustic-workers`；parity 可在训练前 20 会话的标签和 40 路 Zarr 就绪后提前运行。声学 1000/1000 已完成、GPU1 parity 已通过，标签四分片 339/339 已闭合。缺失标签逐通道载入完整波形以收紧峰值内存；摄取 3 worker 与标签 4 个互斥分片重叠。`--stage baselines --device cuda:1` 可在摄取期间提前生成 hazard 与声学 GRU 正式格，主网格按格子断点直接复用。
